...
<script>
 	...
 	function submitForm(){
 		$.ajax({ 
            cache: true, 
            type: "POST", 
            url:"<%=basePath %>/coolMethod/saveEditOrAdd", 
            ...
            success: function(data) {
            	...
            	if(result.succeed){
            		alert("保存成功！");
            		//页面跳转到 管理页面 
            		window.location.href="<%=basePath%>/coolMethod/init";
            	}
            } 
        });
 	}
...
<body>    
    ...
    <div class="easyui-panel" title="冷却方法编辑/新增" style="width:600px">
		<div style="padding:10px 20px 20px 20px">
	    <form id="functionForm" method="post" action="#">
	    	<c:if test="${coolMethod.id !=null}">
	    		<input type="hidden" id="id" value="${coolMethod.id }" name="id">
	    	</c:if>
	    	<table cellpadding="5">
	    		<tr>
	    			<td>中文名称：</td>
	    			<td><input type="text" value="${coolMethod.cName }" id="cName" name="cName" width="150px;"></td>
	    		</tr>
	    		<tr>
	    			<td>英文名称：</td>
	    			<td><input type="text" value="${coolMethod.eName }" id="eName" name="eName" width="150px;"></td>
	    		</tr>
	    	</table>
	    </form>
