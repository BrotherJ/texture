...
<script type="text/javascript">
	...
	function edit(id){
		//跳转到编辑页面
		window.location.href="<%=basePath%>/coolMethod/edit?id="+id;
	}
	
	function del(id){
		var currentPage= ${page.pageNo};
		
		if(confirm("你是否确定要删除该冷却方法？")){
			$.ajax({ 
	            cache: true, 
	            type: "POST", 
	            url:"<%=basePath %>/coolMethod/delCoolMethodById?id="+id, 
	            ...
	            success: function(data) {
	            	...
	            	if(result.succeed){
	            		var cName =$("#cName").val();
	            		var eName =$("#eName").val();
	            		window.location.href="<%=basePath %>/coolMethod/init/?page="+currentPage+"&cName="+cName+"&eName="+eName;
	...
	function addNew(){
		window.location.href="<%=basePath%>/coolMethod/add";
	}
	...
	function search(){
		var cName =$("#cName").val();
		var eName =$("#eName").val();
		window.location.href="<%=basePath %>/coolMethod/init/?page=1&cName="+cName+"&eName="+eName;
	}
</script>
</head>
<body>
	...
	<div id="functionInfo">
		<table>
			<tr>
				...
				<th scope="col" width="150px;">英文名称</th>
