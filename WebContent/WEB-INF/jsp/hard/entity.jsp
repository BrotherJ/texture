...
<script>
 	...
 	function submitForm(){
 		$.ajax({ 
            cache: true, 
            type: "POST", 
            url:"<%=basePath %>/hard/saveEditOrAdd", 
            ...
            success: function(data) {
            	if(data.succeed){
            		alert(data.msg);
            		//页面跳转到 管理页面 
            		window.location.href="<%=basePath%>/hard/init";
            	}else{
...
<body>    
    ...
    <div class="easyui-panel" title="硬度编辑/新增" style="width:600px">
		<div style="padding:10px 20px 20px 20px">
	    <form id="functionForm" method="post" action="#">
	    	<c:if test="${hard.id !=null}">
	    		<input type="hidden" id="id" value="${hard.id }" name="id">
	    	</c:if>
	    	<table cellpadding="5">
	    		<tr>
	    			<td>HB：</td>
	    			<td><input type="text" value="${hard.hb }" id="hb" name="hb" width="150px;"></td>
	    		</tr>
	    		<tr>
	    			<td>HRC：</td>
	    			<td><input type="text" value="${hard.hrc }" id="hrc" name="hrc" width="150px;"></td>
	    		</tr>
	    		<tr>
	    			<td>HV：</td>
	    			<td><input type="text" value="${hard.hv }" id="hv" name="hv" width="150px;"></td>
	    		</tr>
	    	</table>
	    </form>
