...
<script>
 	...
 	function submitForm(){
 		$.ajax({ 
            cache: true, 
            type: "POST", 
            url:"<%=basePath %>/stand/saveEditOrAdd", 
...
<body>    
    ...
    <div class="easyui-panel" title="标准与版本号维护" style="width:600px">
		<div style="padding:10px 20px 20px 20px">
	    <form id="functionForm" method="post" action="#">
	    	<c:if test="${stand.id !=null}">
	    		<input type="hidden" id="id" value="${stand.id }" name="id">
	    	</c:if>
	    	<table cellpadding="5">
	    		<tr>
	    			<td>标准/版本号：</td>
	    			<td><input type="text" value="${stand.name }" id="name" name="name" width="150px;"></td>
	    		</tr>
	    		<tr>
	    			<td>材质：</td>
	    			<td><input type="text" value="${stand.texture }" id="texture" name="texture" width="150px;"></td>
	    		</tr>
	    	</table>
	    </form>
