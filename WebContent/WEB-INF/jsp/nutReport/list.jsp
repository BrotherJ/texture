...
<script type="text/javascript">
			...
			$.ajax({ 
                ...
                success: function(data) {
                	if(data.succeed){
                		...
                		window.location.href="<%=basePath %>/nutReportSearch/initList/?page=1&texture="+texture+"&furnace_No="+furnace_No+"&createUser="+createUser+"&status="+status+"&beginDate="+beginDate+"&endDate="+endDate;
                	}else{
	...
	function goPage(pageNum){
		...
		var beginDate =$('#beginDate').datebox('getValue'); 
		var endDate =$('#endDate').datebox('getValue'); 
		window.location.href="<%=basePath %>/nutReportSearch/initList/?page="+pageNum+"&texture="+texture+"&furnace_No="+furnace_No+"&createUser="+createUser+"&status="+status+"&beginDate="+beginDate+"&endDate="+endDate;
	}
	...
	function goUpload(id){
		...
		var beginDate =$('#beginDate').datebox('getValue'); 
		var endDate =$('#endDate').datebox('getValue'); 
		window.location.href="<%=basePath %>/upload/goUpload?id="+id+"&reportType=1&texture="+texture+"&furnace_No="+furnace_No+"&status="+status+"&beginDate="+beginDate+"&endDate="+endDate;
	}
	...
	function showAddInfo(texture,furnace_No){
		$.ajax({ 
            cache: true, 
            type: "POST", 
            url:"<%=basePath %>/nutReportSearch/getAddInfoByTextureAndFurnaceNo?texture="+encodeURIComponent(encodeURIComponent(texture))+"&furnace_No="+encodeURIComponent(encodeURIComponent(furnace_No)), 
            dataType:"json",
            async: false, 
            error: function(request) { 
            	$.messager.alert('Error','连接失败!');
	...
	function getOperation(textureReport,user){
		...
		if(textureReport.status=="0"){
			...
			if(user.userType=="2"){
				...
				str =str+ "<input type='button'  value='下载' onclick='download(\""+textureReport.id+"\")'>";
				str =str+ "<input type='button'  value='打印' onclick='print(\""+textureReport.id+"\")'>";
				str =str+ "<input type='button'  value='传入sap' onclick='checkExistInSapAndSend(\""+textureReport.id+"\")'>";
				str =str+ "<input type='button'  value='退回' onclick='returnBackAdd(\""+textureReport.id+"\")'>";
			}
