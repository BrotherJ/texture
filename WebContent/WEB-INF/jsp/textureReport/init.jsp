...
<script type="text/javascript">
	...
	$(function(){
		...
		if(id==""){
			...
			disableForm("gwMechanicalTableForm",true);
			...
			//高温机械性能，若勾选,则需要维护高温机械性能
			var isGw = $("#isGw").prop("checked");
			if(isGw){
				disableForm("gwMechanicalTableForm",false);
			}else{
				disableForm("gwMechanicalTableForm",true);
			}
			//炉号不能修改
			$("#furnace_No").attr("readonly",true);
			//将全局变量赋值为自定义元素数量 -- 非追加记录
		...
		function AssignmentAll(textureReport){
			...
			//设置高温机械性能--判断高温机械性能是否为空,若为空则不需要赋值
			if(textureReport.gwMechanical!=null){
				setGwMechanical(textureReport.gwMechanical);
			}
			//化学成分
		...
		function setBasicInfo(textureReport){
			...
			if(textureReport.isNw=="true"){
				//$("#isNw").attr("checked","checked");
				$("#isNw").prop("checked","checked");
			}
			
			//是否需要高温机械性能
			if(textureReport.isGw=="true"){
				$("#isGw").prop("checked","checked");
			}
			
			$("#pren_standard").val(textureReport.pren_standard);
			$("#pren_b").val(textureReport.pren_b);

			//var ss =$("#standAndVersionNum").val();
			//alert(ss);
		}
		
		/**设置高温机械性能**/
		function setGwMechanical(gwMechanical){
			//高温抗拉
			$("#gwkl").val(gwMechanical.gwkl);
			//高温屈服
			$("#gwqf").val(gwMechanical.gwqf);
			//高温延伸
			$("#gwysh").val(gwMechanical.gwysh);
			//高温收缩
			$("#gwss").val(gwMechanical.gwss);
			//温度
			$("#wd").val(gwMechanical.wd);
		}
		
		/**设置机械性能**/
		function setMachinal(mechanical){
			$("#kl").val(mechanical.kl);
			$("#qf").val(mechanical.qf);
			$("#ysh").val(mechanical.ysh);
			$("#ss").val(mechanical.ss);
	...
	function Assignment(result){
		...
		//设置高温机械性能
		//高温抗拉最小值、最大值
		$("#gwkl_min").val(result.textureStandard.gwMechanical.gwkl_min);
		$("#gwkl_max").val(result.textureStandard.gwMechanical.gwkl_max);
		if(result.textureStandard.gwMechanical.gwkl_max=="0.0"){
			$("#gwkl_max").val(9999);
		}else{
			$("#gwkl_max").val(result.textureStandard.gwMechanical.gwkl_max);
		}
		//高温屈服最小值、最大值
		$("#gwqf_min").val(result.textureStandard.gwMechanical.gwqf_min);
		if(result.textureStandard.gwMechanical.gwqf_max=="0.0"){
			$("#gwqf_max").val(9999);
		}else{
			$("#gwqf_max").val(result.textureStandard.gwMechanical.gwqf_max);
		}
		//高温延伸最小值、最大值
		$("#gwysh_min").val(result.textureStandard.gwMechanical.gwysh_min);
		if(result.textureStandard.gwMechanical.gwysh_max=="0.0"){
			$("#gwysh_max").val(9999);
		}else{
			$("#gwysh_max").val(result.textureStandard.gwMechanical.gwysh_max);
		}
		//高温收缩最小值、最大值
		$("#gwss_min").val(result.textureStandard.gwMechanical.gwss_min);
		if(result.textureStandard.gwMechanical.gwss_max=="0.0"){
			$("#gwss_max").val(9999);
		}else{
			$("#gwss_max").val(result.textureStandard.gwMechanical.gwss_max);
		}
		
		//设置机械性能最大值和最小值
		//抗拉最小值、最大值
		$("#kl_min").val(result.textureStandard.mechanical.kl_min);
		$("#kl_max").val(result.textureStandard.mechanical.kl_max);
		if(result.textureStandard.mechanical.kl_max=="0.0"){
			$("#kl_max").val(9999);
		}else{
			$("#kl_max").val(result.textureStandard.mechanical.kl_max);
		}
		//屈服最小值、最大值
		$("#qf_min").val(result.textureStandard.mechanical.qf_min);
		if(result.textureStandard.mechanical.qf_max=="0.0"){
			$("#qf_max").val(9999);
		}else{
			$("#qf_max").val(result.textureStandard.mechanical.qf_max);
		}
		//延伸最小值、最大值
		$("#ysh_min").val(result.textureStandard.mechanical.ysh_min);
		if(result.textureStandard.mechanical.ysh_max=="0.0"){
			$("#ysh_max").val(9999);
		}else{
			$("#ysh_max").val(result.textureStandard.mechanical.ysh_max);
		}
		//收缩最小值、最大值
		$("#ss_min").val(result.textureStandard.mechanical.ss_min);
		if(result.textureStandard.mechanical.ss_max=="0.0"){
			$("#ss_max").val(9999);
		}else{
			$("#ss_max").val(result.textureStandard.mechanical.ss_max);
		}
		//硬度最小值、最大值
	...
	function add(){
		...
		+"<td><input type='text' id='material_name"+material_no+"' name='material_name"+material_no+"' readonly>"
		+"<input type='hidden'  id='material_name_en"+material_no+"' name='material_name_en"+material_no+"' readonly></td>"
		+"<td><input type='text' id='material_spec"+material_no+"' name='material_spec"+material_no+"' readonly></td>"
		+"<td><input type='text' id='num"+material_no+"' name='num"+material_no+"' onblur='checkNum("+material_no+")'></td></tr>";
	...
	/**高温机械性能**/
	function getGwMechanicalInfo(){
		//获取机械性能值 
		var gwkl =$("#gwkl").val();
		var gwkl_min=$("#gwkl_min").val();
		var gwkl_max=$("#gwkl_max").val();
		var gwqf =$("#gwqf").val();
		var gwqf_min =$("#gwqf_min").val();
		var gwqf_max =$("#gwqf_max").val();
		var gwysh =$("#gwysh").val();
		var gwysh_min =$("#gwysh_min").val();
		var gwysh_max =$("#gwysh_max").val();
		var gwss =$("#gwss").val();
		var gwss_min =$("#gwss_min").val();
		var gwss_max =$("#gwss_max").val();
		//温度
		var wd = $("#wd").val();
		
		var gwmechanical_front="gwMechanical:{";
		var gwmechanical_end="},";
		var gwmechanical_json = "gwkl_min:"+gwkl_min+",gwkl_max:"+gwkl_max
		+",gwqf_min:"+gwqf_min+",gwqf_max:"+gwqf_max
		+",gwysh_min:"+gwysh_min+",gwysh_max:"+gwysh_max
		+",gwss_min:"+gwss_min+",gwss_max:"+gwss_max;
		
		if(gwkl!=""){
			gwmechanical_json =gwmechanical_json+",gwkl:'"+gwkl+"'";
		}
		if(gwqf!=""){
			gwmechanical_json =gwmechanical_json+",gwqf:'"+gwqf+"'";
		}
		if(gwysh!=""){
			gwmechanical_json =gwmechanical_json+",gwysh:'"+gwysh+"'";
		}
		if(gwss!=""){
			gwmechanical_json =gwmechanical_json+",gwss:'"+gwss+"'";
		}
		if(wd!=""){
			gwmechanical_json =gwmechanical_json+",wd:'"+wd+"'";
		}
		var str = gwmechanical_front+gwmechanical_json+gwmechanical_end;
		return str;
	}
	...
	function getMechanical(){
		//获取机械性能值 
		var kl =$("#kl").val();
		var kl_min=$("#kl_min").val();
		var kl_max=$("#kl_max").val();
		var qf =$("#qf").val();
		var qf_min =$("#qf_min").val();
		var qf_max =$("#qf_max").val();
		var ysh =$("#ysh").val();
		var ysh_min =$("#ysh_min").val();
		var ysh_max =$("#ysh_max").val();
		var ss =$("#ss").val();
		var ss_min =$("#ss_min").val();
		var ss_max =$("#ss_max").val();
		var yd =$("#yd").val();
		...
		var mechanical_end="},";
		var mechanical_json = "kl_min:"+kl_min+",kl_max:"+kl_max
		+",qf_min:"+qf_min+",qf_max:"+qf_max
		+",ysh_min:"+ysh_min+",ysh_max:"+ysh_max
		+",ss_min:"+ss_min+",ss_max:"+ss_max
		+",yd_min:"+yd_min+",yd_max:"+yd_max+",yddw:'"+yddw+"'";
		
		if(kl!=""){
			mechanical_json =mechanical_json+",kl:'"+kl+"'";
		}
		if(qf!=""){
			mechanical_json =mechanical_json+",qf:'"+qf+"'";
		}
		if(ysh!=""){
			mechanical_json =mechanical_json+",ysh:'"+ysh+"'";
		}
		if(ss!=""){
			mechanical_json =mechanical_json+",ss:'"+ss+"'";
		}
	...
	function replaceMechanical(mechanical){
		//设置机械性能最大值和最小值
		//抗拉最小值、最大值
		$("#kl_min").val(mechanical.kl_min);
		$("#kl_max").val(mechanical.kl_max);
		if(mechanical.kl_max=="0.0"){
			$("#kl_max").val(9999);
		}else{
			$("#kl_max").val(mechanical.kl_max);
		}
		//屈服最小值、最大值
		$("#qf_min").val(mechanical.qf_min);
		if(mechanical.qf_max=="0.0"){
			$("#qf_max").val(9999);
		}else{
			$("#qf_max").val(mechanical.qf_max);
		}
		//延伸最小值、最大值
		$("#ysh_min").val(mechanical.ysh_min);
		if(mechanical.ysh_max=="0.0"){
			$("#ysh_max").val(9999);
		}else{
			$("#ysh_max").val(mechanical.ysh_max);
		}
		//收缩最小值、最大值
		$("#ss_min").val(mechanical.ss_min);
		if(mechanical.ss_max=="0.0"){
			$("#ss_max").val(9999);
		}else{
			$("#ss_max").val(mechanical.ss_max);
		}
		//硬度最小值、最大值
	...
	function submitForm(){
		...
		if(isGw){
			//拼接json
			gw_mechanical_info =getGwMechanicalInfo();
			var out_gw = checkGwMechanicalInfo();
			if(out_gw!=""){
				//alert(out_material);
				$.messager.alert('Error',out_gw);
				return false;
			}
		}
		...
		var jsonStr= front+texture_info+formula_info+element_info+cus_element_info+mechanical_info+gw_mechanical_info+material_info+heatInfo+checkInfo+end;
		...
		if (clicktag == 0) {  
			...
			$.ajax({ 
	            cache: true, 
	            type: "POST", 
	            url:"<%=basePath %>/textureReport/addTextureReport?id="+id, 
	            data:{'mydata':jsonStr},
	            dataType:"json",
	            async: false, 
	            error: function(request) { 
	            ...
	            success: function(data) {
	            	if(data.succeed){
	            		alert("保存成功!");
	           			window.location.href="<%=basePath %>/textureReportSearch/initList/?page=1&status=0";
	            	}else{
	            		clicktag = 0;
	            		alert(data.msg);
	            	}
	...
	/**验证高温机械性能**/
	function checkGwMechanicalInfo(){
		var out="";
		//获取机械性能值 
		var gwkl =$("#gwkl").val();
		var gwkl_min=$("#gwkl_min").val();
		...
		//判断上下限是否为0-9999，若是，默认可以不需要输入值
		if(!(parseInt(gwkl_min)==0&&parseInt(gwkl_max)==9999)){
			//判断是否为空，不允许为空
			if(gwkl=="" || !checkRate(gwkl)){
				out = out+"高温抗拉值不允许为空或输入字母!\n<br>";
			}
		}
		if(!(parseInt(gwqf_min)==0 &&parseInt(gwqf_max)==9999)){
			if(gwqf==""|| !checkRate(gwqf)){
				out = out+"高温屈服值不允许为空或输入字母!\n<br>";
			}
		}
		if(!(parseInt(gwysh_min)==0&&parseInt(gwysh_max)==9999)){
			if(gwysh==""|| !checkRate(gwysh)){
				out = out+"高温延伸值不允许为空或输入字母!\n<br>";
			}
		}
		if(!(parseInt(gwss_min)==0&&parseInt(gwss_max)==9999)){
			if(gwss==""|| !checkRate(gwss)){
				out = out+"高温收缩值不允许为空或输入字母!\n<br>";
			}
		}
		
		//判断实际温度必须是整数
		if(wd!=""&& !validateNum(wd.replace("℃",""))){
			out = out+"温度只允许输入数字!\n<br>";
		}
		//判断抗拉值是否在规定范围内 --强制转换成float型进行判断大小
		if(parseFloat(gwkl)>parseFloat(gwkl_max) || parseFloat(gwkl)<parseFloat(gwkl_min)){
			out = out+"高温抗拉值不在规定范围内,最大值:"+gwkl_max+",最小值:"+gwkl_min+",请重新输入!\n<br>";
		}
		//判断屈服值是否在规定范围内
		if(parseFloat(gwqf)>parseFloat(gwqf_max) || parseFloat(gwqf)<parseFloat(gwqf_min)){
			out = out+"高温屈服值不在规定范围内,最大值:"+gwqf_max+",最小值:"+gwqf_min+",请重新输入!\n<br>";
		}
		//判断延伸值是否在规定范围内
		if(parseFloat(gwysh)>parseFloat(gwysh_max) || parseFloat(gwysh)<parseFloat(gwysh_min)){
			out = out+"高温延伸值不在规定范围内,最大值:"+gwysh_max+",最小值:"+gwysh_min+",请重新输入!\n<br>";
		}
		//判断收缩值是否在规定范围内
		if(parseFloat(gwss)>parseFloat(gwss_max) || parseFloat(gwss)<parseFloat(gwss_min)){
			out = out+"高温收缩值不在规定范围内,最大值:"+gwss_max+",最小值:"+gwss_min+",请重新输入!\n<br>";
		}
	...
	function checkMechanical(){
		...
		var out ="";
		//判断上下限是否为0-9999，若是，默认可以不需要输入值
		if(!(parseInt(kl_min)==0&&parseInt(kl_max)==9999)){
			//判断是否为空，不允许为空
			if(kl=="" || !checkRate(kl)){
				out = out+"抗拉值不允许为空或输入字母!\n<br>";
			}
		}
		if(!(parseInt(qf_min)==0 &&parseInt(qf_max)==9999)){
			if(qf==""|| !checkRate(qf)){
				out = out+"屈服值不允许为空或输入字母!\n<br>";
			}
		}
		if(!(parseInt(ysh_min)==0&&parseInt(ysh_max)==9999)){
			if(ysh==""|| !checkRate(ysh)){
				out = out+"延伸值不允许为空或输入字母!\n<br>";
			}
		}
		if(!(parseInt(ss_min)==0&&parseInt(ss_max)==9999)){
			if(ss==""|| !checkRate(ss)){
				out = out+"收缩值不允许为空或输入字母!\n<br>";
			}
		}
		...
		//判断抗拉值是否在规定范围内 --强制转换成float型进行判断大小
		if(parseFloat(kl)>parseFloat(kl_max) || parseFloat(kl)<parseFloat(kl_min)){
			out = out+"抗拉值不在规定范围内,最大值:"+kl_max+",最小值:"+kl_min+",请重新输入!\n<br>";
		}
		//判断屈服值是否在规定范围内
		if(parseFloat(qf)>parseFloat(qf_max) || parseFloat(qf)<parseFloat(qf_min)){
			out = out+"屈服值不在规定范围内,最大值:"+qf_max+",最小值:"+qf_min+",请重新输入!\n<br>";
		}
		//判断延伸值是否在规定范围内
		if(parseFloat(ysh)>parseFloat(ysh_max) || parseFloat(ysh)<parseFloat(ysh_min)){
			out = out+"延伸值不在规定范围内,最大值:"+ysh_max+",最小值:"+ysh_min+",请重新输入!\n<br>";
		}
		//判断收缩值是否在规定范围内
		if(parseFloat(ss)>parseFloat(ss_max) || parseFloat(ss)<parseFloat(ss_min)){
			out = out+"收缩值不在规定范围内,最大值:"+ss_max+",最小值:"+ss_min+",请重新输入!\n<br>";
		}
		if(!(yd_min=="0" && yd_max==0)){
			if(yd==""|| !checkRate(yd)){
			...
			if(yddw=="HB"){
				...
				//对于单位为HRC时，取该hrc值对应的HB值，并判断是否在最大值最小值范围内
	...
	function getMaterialInfo(){
		...
		for (var i = 1; i <= material_len; i++) {
			...
			num = $("#num"+i).val();
			material_info =material_info+"{material_code:'"+material_code+"',material_name:'"+material_name+"',material_name_en:'"+material_name_en+"',material_spec:'"+material_spec+"',num:"+num+"},";
		}
		//去除最后一位逗号
	...
	function getTextureInfo(){
		...
		//判断isNw是否不存在
		if(isNw!=undefined){
			texture_info =texture_info+",isNw:"+isNw;
		}else{
			//若neway用户登录，没有复选框，但是是需要neway维护的
			texture_info =texture_info+",isNw:"+ck_isNw;
		}
		//判断是否需要高温机械性能
		if(isGw!=undefined){
			texture_info =texture_info+",isGw:"+isGw;
		}
...
<body>
		<div class="easyui-accordion" id="accordion" name="accordion" data-options="multiple:true" style="width:98%;">
			...
			<form action="#" id="textureReport">
		    	<table cellpadding="5">
		    			...
		    			<td>
	    					...
	    					<select id=standAndVersionNum name='standAndVersionNum'>
		    						...
		    						<!--<c:forEach items="${stands}" var="stand">
		    							<option value="${stand.name }" <c:if test='${textureReport.standAndVersionNum ==stand.name}'>selected</c:if> >${stand.name }</option>
		    						</c:forEach>-->
		    				</select>
		    			</td>
		    		</tr>
		    		...
		    		<tr>
		    			...
		    			<td>
		    				...
		    				<input type="checkbox" id="isGw" name="isGw"  <c:if test="${textureReport.isGw==true}">checked</c:if>>是否维护高温机械性能
		    			</td>
		    		</tr>
		    		<tr>
		    			<td></td>
		    	...
		    	<!-- 是否追加标志 -->
		    	<input type="hidden" id="addSign" name="addSign"  value="${textureReport.addSign }">
		    	<!-- 记录上传信息 -->
		    	<input type="hidden" id="uploadSign" name="uploadSign" value="${textureReport.uploadSign }">
		    	<input type="hidden" id="uploadUrl" name="uploadUrl" value="${textureReport.uploadUrl }">
		    	<!-- 记录是否传人sap -->
				    	...
				    	<tbody>
				    		<c:forEach items="${textureReport.materials}" var="material" varStatus="status">
				    			<tr>
				    				...
				    				<td>
				    					<input type="text" id="material_spec${status.index+1 }" name="material_spec${status.index+1 }" value="${material.material_spec }" readonly>
				    				</td>
				    				<td>
				    					<input type="text" id="num${status.index+1 }" name="num${status.index+1 }" value="${material.num }" onblur='checkNum("${status.index+1 }")'>
				    				</td>
				    			</tr>
				    		</c:forEach>
				    	</tbody>
			   ...
			   <form action="#" id="mechanicalTableForm">
			    <table id="mechanicalTable">
					<tr>
						<td>抗拉</td>
						<td>
							<input type="text" id="kl" name="kl" style="width: 100px;" value="${textureReport.mechanical.kl }">
							<input type="hidden" id="kl_min" name="kl_min" value="${textureReport.mechanical.kl_min}">
							<input type="hidden" id="kl_max" name="kl_max" value="${textureReport.mechanical.kl_max}">
						</td>
						<td>屈服</td>
						<td>
							<input type="text" id="qf" name="qf" style="width: 100px;" value="${textureReport.mechanical.qf }">
							<input type="hidden" id="qf_min" name="qf_min" style="width: 100px;" value="${textureReport.mechanical.qf_min }">
							<input type="hidden" id="qf_max" name="qf_max" style="width: 100px;" value="${textureReport.mechanical.qf_max }">
						</td>
						<td>延伸</td>
						<td>
							<input type="text" id="ysh" name="ysh" style="width: 100px;" value="${textureReport.mechanical.ysh }">
							<input type="hidden" id="ysh_min" name="ysh_min" style="width: 100px;" value="${textureReport.mechanical.ysh_min }">
							<input type="hidden" id="ysh_max" name="ysh_max" style="width: 100px;" value="${textureReport.mechanical.ysh_max }">
						</td>
						<td>收缩</td>
						<td>
							<input type="text" id="ss" name="ss" style="width: 100px;" value="${textureReport.mechanical.ss }">
							<input type="hidden" id="ss_min" name="ss_min" style="width: 100px;" value="${textureReport.mechanical.ss_min }">
							<input type="hidden" id="ss_max" name="ss_max" style="width: 100px;" value="${textureReport.mechanical.ss_max }">
						</td>
					</tr>
			...
			<div title="高温机械性能"  id="gwMechanicalDiv" data-options="iconCls:'icon-ok'"  style="overflow:auto;padding:10px;">		    
			   <form action="#" id="gwMechanicalTableForm">
			    <table id="mechanicalTable">
					<tr>
						<td>抗拉</td>
						<td>
							<input type="text" id="gwkl" name="kl" style="width: 100px;" value="${textureReport.gwMechanical.gwkl }">
							<input type="hidden" id="gwkl_min" name="gwkl_min" value="${textureReport.gwMechanical.gwkl_min}">
							<input type="hidden" id="gwkl_max" name="gwkl_max" value="${textureReport.gwMechanical.gwkl_max}">
						</td>
						<td>屈服</td>
						<td>
							<input type="text" id="gwqf" name="gwqf" style="width: 100px;" value="${textureReport.gwMechanical.gwqf }">
							<input type="hidden" id="gwqf_min" name="gwqf_min" style="width: 100px;" value="${textureReport.gwMechanical.gwqf_min }">
							<input type="hidden" id="gwqf_max" name="gwqf_max" style="width: 100px;" value="${textureReport.gwMechanical.gwqf_max }">
						</td>
						<td>延伸</td>
						<td>
							<input type="text" id="gwysh" name="gwysh" style="width: 100px;" value="${textureReport.gwMechanical.gwysh }">
							<input type="hidden" id="gwysh_min" name="gwysh_min" style="width: 100px;" value="${textureReport.gwMechanical.gwysh_min }">
							<input type="hidden" id="gwysh_max" name="gwysh_max" style="width: 100px;" value="${textureReport.gwMechanical.gwysh_max }">
						</td>
						<td>收缩</td>
						<td>
							<input type="text" id="gwss" name="gwss" style="width: 100px;" value="${textureReport.gwMechanical.gwss }">
							<input type="hidden" id="gwss_min" name="gwss_min" style="width: 100px;" value="${textureReport.gwMechanical.gwss_min }">
							<input type="hidden" id="gwss_max" name="gwss_max" style="width: 100px;" value="${textureReport.gwMechanical.gwss_max }">
						</td>
					</tr>
					...
					<tr>
						<td>温度</td>
						<td>
							<input type="text" id="wd" name="wd" style="width: 100px;" value="${textureReport.gwMechanical.wd }">
						</td>
					</tr>
				</table>
				</form>	
		...
		<div style="text-align:center;padding:5px">
	    	<a href="javascript:void(0)" class="easyui-linkbutton" id="sub" onclick="submitForm()">保存</a>
	    </div>
