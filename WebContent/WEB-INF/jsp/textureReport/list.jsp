...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#returnback").click(function(){
			...
			var id=$("#id").val();
			var backSign =$('input[name="backSign"]:checked').val();
			...
			$.ajax({ 
                cache: true, 
                type: "POST", 
                url:"<%=basePath %>/status/returnback?id="+id+"&remark="+encodeURIComponent(encodeURIComponent(remark))+"&backSign="+backSign, 
                dataType:"json",
                async: false, 
                ...
                success: function(data) {
                	if(data.succeed){
                		...
                		window.location.href="<%=basePath %>/textureReportSearch/initList/?page=1&texture="+texture+"&furnace_No="+furnace_No+"&createUser="+createUser+"&status="+status+"&beginDate="+beginDate+"&endDate="+endDate;
                	}else{
	...
	function goPage(pageNum){
		...
		var beginDate =$('#beginDate').datebox('getValue'); 
		var endDate =$('#endDate').datebox('getValue'); 
		window.location.href="<%=basePath %>/textureReportSearch/initList/?page="+pageNum+"&texture="+texture+"&furnace_No="+furnace_No+"&createUser="+createUser+"&status="+status+"&beginDate="+beginDate+"&endDate="+endDate;
	}
	...
	function edit(id){
		window.location.href="<%=basePath %>/textureReport/edit?id="+id;
	}
	
	/**新增**/
	function addNew(){
		window.location.href="<%=basePath %>/textureReport/init";
	}
	...
	function goUpload(id){
		...
		window.location.href="<%=basePath %>/upload/goUpload?id="+id+"&reportType=0&texture="+texture+"&furnace_No="+furnace_No+"&status="+status+"&beginDate="+beginDate+"&endDate="+endDate;
	}
	...
	function delAttach(id){
		$.ajax({ 
            cache: true, 
            type: "POST", 
            url:"<%=basePath %>/status/delAttach?id="+id, 
            dataType:"json",
            async: false, 
            error: function(request) { 
            	$.messager.alert('Error','连接失败!');
	...
	function submitDraft(id){
		$.ajax({ 
            cache: true, 
            type: "POST", 
            url:"<%=basePath %>/status/submitDraft?id="+id, 
            dataType:"json",
            async: false, 
            error: function(request) { 
            	$.messager.alert('Error','连接失败!');
	...
	function delDraft(id){
		$.ajax({ 
            cache: true, 
            type: "POST", 
            url:"<%=basePath %>/status/delDraft?id="+id, 
            dataType:"json",
            async: false, 
            error: function(request) { 
            	$.messager.alert('Error','连接失败!');
	...
	/**检查材质 炉号是否已存在 并传入sap**/
	function checkExistInSapAndSend(id){
		/**将材质信息传入sap**/
		$.ajax({ 
            cache: true, 
            type: "POST", 
            url:"<%=basePath %>/status/checkExistInSap?id="+id,
            dataType:"json",
            async: false, 
            error: function(request) { 
            	$.messager.alert('Error','连接失败!');
	...
	function sendSap(id){
		...
		$.ajax({ 
            cache: true, 
            type: "POST", 
            url:"<%=basePath %>/status/sendToSap?id="+id,
            dataType:"json",
            async: false, 
            error: function(request) { 
            	$.messager.alert('Error','连接失败!');
	...
	/**追加记录退回**/
	function returnBackAdd(id){
		$.messager.defaults = { ok: "是", cancel: "否" };
		$.messager.confirm("操作提示", "您确定要退回该追加的材质报告吗？", function (data) {
            if (data) {
            	$("#add_id").val(id);
            	$('#addSignBack').window('open');
            }
        });
	}
	...
	function withdraw(id){
		$.messager.defaults = { ok: "是", cancel: "否" };
		$.messager.confirm("操作提示", "您确定要撤回该材质报告吗？", function (data) {
            if (data) {
            	$.ajax({ 
                    cache: true, 
                    type: "POST", 
                    url:"<%=basePath %>/status/withdraw?id="+id, 
                    dataType:"json",
                    async: false, 
                    error: function(request) { 
                    	$.messager.alert('Error','连接失败!');
	...
	function cancel(id){
		$.messager.defaults = { ok: "是", cancel: "否" };
		$.messager.confirm("作废提示", "您确定要作废该材质报告数据信息吗？", function (data) {
            if (data) {
            	$.ajax({ 
                    cache: true, 
                    type: "POST", 
                    url:"<%=basePath %>/status/cancel?id="+id, 
                    dataType:"json",
                    async: false, 
                    error: function(request) { 
                    	$.messager.alert('Error','连接失败!');
	...
	function showAddInfo(texture,furnace_No){
		$.ajax({ 
            cache: true, 
            type: "POST", 
            url:"<%=basePath %>/textureReportSearch/getAddInfoByTextureAndFurnaceNo?texture="+encodeURIComponent(encodeURIComponent(texture))+"&furnace_No="+encodeURIComponent(encodeURIComponent(furnace_No)), 
            dataType:"json",
            async: false, 
            error: function(request) { 
            	$.messager.alert('Error','连接失败!');
	...
	function makeTableNoResult(){
		...
		var str = "<tr><td colspan='11' align='center'>该材质报告无追加记录</td></tr>";
	...
	function makeTable(addReports,user){
		...
		for(var i =0;i<addReports.length;i++){
			...
			"<td class='tdsize'>"+nullToStr(addReports[i].confirmUser)+"</td>"+
			"<td class='tdsize'>"+nullToStr(addReports[i].confirmTime)+"</td>"+
			"<td class='tdsize'>"+getStatus(addReports[i].status)+"</td>"+
			"<td class='tdsize'>"+getUploadSign(addReports[i].uploadSign)+"</td>"+
	...
	function getOperation(textureReport,user){
		...
		if(textureReport.status=="0"){
			...
			if(user.userType=="2"){
				//供应商
				//str =str+ "<input type='button'  value='编辑' onclick='edit(\""+textureReport.id+"\")'>";
				...
				if(textureReport.uploadSign != 1){
					...
					str =str+ "<input type='button'  value='删除附件' onclick='delAttach(\""+textureReport.id+"\")'>";
				}
				//str =str+ "<input type='button'  value='打印' onclick='print(\""+textureReport.id+"\")'>";
				str =str+ "<input type='button'  value='提交' onclick='submitDraft(\""+textureReport.id+"\")'>";
				str =str+ "<input type='button'  value='删除' onclick='delDraft(\""+textureReport.id+"\")'>";
			}
			...
			if(user.userType=="2"){
				...
				str =str+ "<input type='button'  value='查看' onclick='view(\""+textureReport.id+"\")'>";
				str =str+ "<input type='button'  value='打印' onclick='print(\""+textureReport.id+"\")'>";
				//str =str+ "<input type='button'  value='撤回' onclick='withdraw(\""+textureReport.id+"\")'>";
			}else if(user.userType=="1"){
				//Neway用户
				str =str+ "<input type='button'  value='下载' onclick='download(\""+textureReport.id+"\")'>";
				str =str+ "<input type='button'  value='打印' onclick='print(\""+textureReport.id+"\")'>";
				str =str+ "<input type='button'  value='确认' onclick='confirmTextureReport(\""+textureReport.id+"\")'>";
				str =str+ "<input type='button'  value='退回' onclick='returnBackAdd(\""+textureReport.id+"\")'>";
			}
	...
	/**将是否上传附件由boolean输出为中文**/
	function getUploadSign(uploadSign){
		var str="";
		if(uploadSign=="1"){
			str="是";
		}else{
...
<body>
		...
		<h3 style="margin-bottom: 0px;margin-top: 0px">零件材质报告</h3>
	...
	<div id="textureInfo"  style="width:1300px;overflow: auto">
					...
					<td class="td">
						<c:if test="${textureReport.status==0 }">
							<c:if test="${user.userType=='2' }">
								...
								<c:if test="${textureReport.backSign != 0 }">
									<input type="button" value="编辑"  onclick='edit("${textureReport.id}")'  >
								</c:if>
								<c:if test="${textureReport.backSign == 0 }">
									<input type="button" value="查看"   onclick='view("${textureReport.id}")'>
								</c:if>
								<!-- 若已上传过，不需要显示上传按钮 -->
								<c:if test="${textureReport.uploadSign != 1 }">
									<input type="button" value="上传" onclick='goUpload("${textureReport.id}")'  >
								</c:if>
								<c:if test="${textureReport.uploadSign == 1 }">
									<input type="button" value="下载"  onclick='download("${textureReport.id}")' >
									<input type="button" value="删除附件"  onclick='delAttach("${textureReport.id}")' >
								</c:if>
								<input type="button" value="打印"   onclick='print("${textureReport.id}")'> 
								<input type="button" value="提交"  onclick='submitDraft("${textureReport.id}")' >
								<input type="button" value="删除"  onclick='delDraft("${textureReport.id}")' >
							</c:if>
						</c:if>
						<!-- 待确认 -->
						...
						<c:if test="${textureReport.status==2 }">
							...
							<c:if test="${user.userType=='1' }">
								<input type="button" value="下载"  onclick='download("${textureReport.id}")' >
								<input type="button" value="打印"  onclick='print("${textureReport.id}")'>
								<input type="button" value="传sap"  onclick='checkExistInSapAndSend("${textureReport.id}")' >
								<input type="button" value="退回"  onclick='returnBack("${textureReport.id}")' >
							</c:if>
						</c:if>
						<!-- 已传入 -->
						<c:if test="${textureReport.status==3 }">
							...
							<c:if test="${user.userType=='1' }">
								...
								<input type="button" value="打印" onclick='print("${textureReport.id}")'>
								<input type="button" value="退回" onclick='returnBack("${textureReport.id}")' >
								<input type="button" value="作废"  onclick='cancel("${textureReport.id}")' >
							</c:if>
						</c:if>
						<!-- 已作废 -->
	...
	<div id="w" class="easyui-window" title="追加记录清单" data-options="modal:true,closed:true,iconCls:'icon-save'" style="width:1100px;height:400px;padding:10px;">
		<div>
			<table id="searchResult">
				...
				<tr>
					...
					<th scope="col" style="width: 45px;">创建日期</th>
					<th scope="col" style="width: 65px;">确认人</th>
					<th scope="col" style="width: 68px;">确认日期</th>
					<th scope="col" style="width: 35px;">状态</th>
					<th scope="col" style="width: 45px;">附件上传</th>
					<th scope="col" style="width: 45px;">纽威维护</th>
