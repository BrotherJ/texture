	...
	public static void createPdfReport(OutputStream outputStream,TextureReport textureReport,User currentUser){
		 Document document = new Document(PageSize.A4.rotate(),10,10,20,10);
	        try { 
	         ...
	         //螺母的保证载荷
//	         NutProof nutProof =textureReport.getNutProof();
	         //ce pren 值
	         String ce = textureReport.getCe();
	         String pren =textureReport.getPren();
	         
	         //判断物料数量是否大于4，若大于4，则需分页处理
	         if(materials.size()<=4){
	        	  //第一页
	             document.newPage();
	             if(user!=null){
		         ...
		         //元素、炉号、零件表格 具体值 表格
		         setDetailData(document,fontPT_S,elements,customizeElements,mechanical,materials,ce,pren,textureReport.getzMtrCodeVOs());
		         //热处理表头
	        	 ...
	        	 //对于物料数量大于4 ，第一页显示1-4，第二页显示5-8条，依次类推
	        	 //应显示页数s
	        	 ...
	        	 for (int i = 0; i < s; i++) {
	        		 ...
	        		 if(i==0){
	        			for (int j = 0; j < 4; j++) {
	        			 ...
	        			 for (int j = 4*i; j < materials.size(); j++) {
	        				 pageMaterials.add(materials.get(j));
						}
	        		 }else{
	...
	public static void setDetailInfo(Document document,Font fontPT_S,Font fontPT,List<texture.domain.Element> elements,List<CustomizeElement> customizeElements,Mechanical mechanical,TextureReport textureReport){
		try{
			...
			if(customizeElements!=null){
				...
				table_left =new PdfPTable(5+elements.size()+customizeElementSize+8);
			}else{
				table_left =new PdfPTable(5+elements.size()+8);
			}
...
//	        //设置高度
	        ...
	        for (int i = 0; i < elements.size(); i++) {
	        	...
	        	setCell(cell_i, fontPT_S, 2, 0, 35, true, true);
	        ...
	        if(customizeElements!=null){
		        for (int i = 0; i <customizeElements.size(); i++) {
		        	...
		        	setCell(cell_i, fontPT_S, 2, 0, 0, true, true);
		        	table_left.addCell(cell_i);
				}
	        }
	      	//CE
	        PdfPCell cell_ce =new PdfPCell(new Paragraph("CE",fontPT_S));
	        setCell(cell_ce, fontPT_S, 2, 0, 0, true, true);
	        table_left.addCell(cell_ce);
	        //PREN
	        PdfPCell cell_pren =new PdfPCell(new Paragraph("PREN",fontPT_S));
	        setCell(cell_pren, fontPT_S, 2, 0, 0, true, true);
	        table_left.addCell(cell_pren);
	        
	        
	        //螺母保证载荷
	        PdfPCell cell_bzzh =new PdfPCell(new Paragraph("螺母保证载荷\n Nut proof Tvest\n（LBF or KN or N)",fontPT_S));
	        setCell(cell_bzzh, fontPT_S, 0, 2, 25, true, true);
	        table_left.addCell(cell_bzzh);
//	        //六角
//	        PdfPCell cell_lj =new PdfPCell(new Paragraph("六角\n Hex",fontPT_S));
//	        setCell( cell_lj, fontPT_S, 0, 0, 0, true, true);
//	        table_left.addCell(cell_lj);
	   
	        //硬度
	        PdfPCell cell_yd =new PdfPCell(new Paragraph("硬度 \nHardness\n(HB)",fontPT_S));
	        setCell( cell_yd, fontPT_S, 2, 0, 0, true, true);
	        ...
	        setCell( cell_cj, fontPT_S, 2, 3, 0, true, true);
	        table_left.addCell(cell_cj);
	        
	        //六角/厚六角
	        //厚六角
	        PdfPCell cell_hlj =new PdfPCell(new Paragraph("厚六角\n Hex Hex",fontPT_S));
	        setCell( cell_hlj, fontPT_S, 0, 0, 25, true, true);
	        table_left.addCell(cell_hlj);
	        //六角
	        PdfPCell cell_lj =new PdfPCell(new Paragraph("六角\n Hex",fontPT_S));
	        setCell( cell_lj, fontPT_S, 0, 0,25, true, true);
	        table_left.addCell(cell_lj);
        	...
        	//PREN 
         	...
         	table_left.addCell(cell_pren_Max);
         	 ...
         	 //厚六角_max
	        PdfPCell cell_hlj_max =new PdfPCell();
	        setCell( cell_hlj_max, fontPT_S, 0, 0, 0, true, true);
	        table_left.addCell(cell_hlj_max);
	        //六角_max
	        PdfPCell cell_lj_max=new PdfPCell();
	        setCell( cell_lj_max, fontPT_S, 0, 0, 0, true, true);
	        table_left.addCell(cell_lj_max);
        	...
        	if(zMtrCodeVOs!=null){
         	...
         	table_left.addCell(cell_pren_Min);
         	 ...
         	 //厚六角_min
	        PdfPCell cell_hlj_min =new PdfPCell();
	        setCell( cell_hlj_min, fontPT_S, 0, 0, 0, true, true);
	        table_left.addCell(cell_hlj_min);
	        //六角_max
	        PdfPCell cell_lj_min=new PdfPCell();
	        setCell( cell_lj_min, fontPT_S, 0, 0, 0, true, true);
	        table_left.addCell(cell_lj_min);
	...
	public static void setDetailData(Document document,Font fontPT_S,List<texture.domain.Element> elements,List<CustomizeElement> customizeElements,Mechanical mechanical,List<Material> materials,String ce,String pren,List<ZMtrCodeVO> zMtrCodeVOs){
		try{
			...
			PdfPTable table_left =null;
			if(customizeElements!=null){
				table_left =new PdfPTable(5+elements.size()+customizeElements.size()+8);
			}else{
				table_left =new PdfPTable(5+elements.size()+8);
			}
			table_left.setWidthPercentage(100);
			...
			for (int i = 0; i < materials.size(); i++) {
				...
				setCell( cell_i_wl, fontPT_S, 0, 0, 28, true, true);
				...
				if ("true".equals(ce.toString())) {
					String ce_value = new NutPdfReport().calculateCE(zMtrCodeVOs,elements,customizeElements);
				...
				if ("true".equals(pren.toString())) {
					String pren_value = new NutPdfReport().calculatePREN(zMtrCodeVOs,elements);
				...
				//厚六角
				PdfPCell cell_i_ls =null;
				if(materials.get(i).getNutProof().getHexHex()!=null){
					cell_i_ls=new PdfPCell(new Paragraph(StrUtil.nullToStr(materials.get(i).getNutProof().getHexHex())+" "+StrUtil.nullToStr(materials.get(i).getNutProof().getUnit()),fontPT_S));
				}else{
					cell_i_ls=new PdfPCell();
				}
				setCell( cell_i_ls, fontPT_S, 0, 0, 0, true, true);
				table_left.addCell(cell_i_ls);
				
				//六角
				PdfPCell cell_i_hlj =null;
				if(materials.get(i).getNutProof().getHex()!=null){
					cell_i_hlj=new PdfPCell(new Paragraph(StrUtil.nullToStr(materials.get(i).getNutProof().getHex())+" "+StrUtil.nullToStr(materials.get(i).getNutProof().getUnit()),fontPT_S));
				}else{
					cell_i_hlj=new PdfPCell();
				}
				
				setCell(cell_i_hlj, fontPT_S, 0, 0, 0, true, true);
				table_left.addCell(cell_i_hlj);
...
//			table.addCell(cell8);
	...
	public static float[] getCellWidth(List<texture.domain.Element> elements,List<CustomizeElement> customizeElements){
		...
		if(customizeElements!=null){
			customizeElementSize= customizeElements.size();
			arr=	new float[5+elements.size()+customizeElementSize+8];
		}else{
			arr= new float[5+elements.size()+8];
		}
		arr[0] = 5f;
		arr[1] = 8f;
		arr[2] = 10f;
